...
H\Delta{x}=b \tag{2}
$$
因为增量方程跟优化问题息息相关，优化问题受到零空间不可观的影响，导致了最终求解出来的增量产生了漂移，也就是说，根据公式(2)求解出来的增量可能是$\Delta{x_1}$，也可能是$\Delta{x_2}$，不管是哪一个，都满足上述的增量方程。所以，笔者认为其实零空间对于优化问题的影响其实是影响到了我们求解的增量——李代数。
...
2. 再次重申一遍，零空间是整个优化问题的零空间，其通过优化问题间接的影响节点的优化，正是因为这样，DSO在构建矩阵N的时候，其实是把所有的李代数与DSO优化问题的零空间进行求导的，这块儿具体可以看一下代码；
